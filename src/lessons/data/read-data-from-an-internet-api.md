...
Remember that websites are built with **HTML**. Your browser sends a request for a certain URL of the page you are on. The destination computer, usually hundreds of miles far away from us, that sends the results back to us, is called a server.  The internet server then responds to the request by sending over html code. The browser then interprets this **HTML** code and shows it on the screen.
...
Our Python code is a "client" that sends a request to a server. The server sends us a response back.
...
Let's imagine, as an example, that I am growing a tomato plant in a big pot. From my experience growing plants, I know that tomato plants shouldn't be left in cold weather outside, because this can hurt the plant. I'm living in an area where the weather gets cold sometimes.
...
Many APIs even send back data in JSON format, which is perfect for us!
